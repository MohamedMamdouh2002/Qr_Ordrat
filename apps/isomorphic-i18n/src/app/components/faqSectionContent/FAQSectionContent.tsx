      ...
      <div className={style.faqContainer}>
        ...
        <div className="md:w-10/12 w-11/12 mb-10 gap-20 mx-auto">  
          <div className="col-span-6 pt-5">
            {faqs.map((faq, index) => (
              <div key={faq.id} className="mb-8 relative rounded-xl shadow-lg border border-mainColor border-l-0">
                  <button
                      ...
                      className={`relative 4xl:h-32 bg-white dark:bg-secDark dark:text-white font-monbold flex justify-between items-center w-full ps-0 p-4 border h-auto md:h-min-[90px]
                          ${openAccordion === faq.id ? 'rounded-b-none rounded-xl border-b-0 border-s-8 border-t-0 border-e-0' : 'border-b-0 border-s-8 border-t-0 border-e-0 rounded-xl'}`}
                  >
                      <span className="md:text-base 4xl:text-xl text-start text-base font-montserrat ps-5 dark:text-white">{faq.question}</span>
                      <FontAwesomeIcon className="4xl:text-2xl ps-2" icon={openAccordion === faq.id ? faAngleUp : faAngleDown} />
                  </button>
                  <div
                      style={{ borderColor: index % 2 === 0 ? '#f97316' : '#5B5B5B' }} 
                      className={`top-0 end-0 grid overflow-hidden 4xl:text-2xl transition-all text-md text-[#747794] dark:bg-secDark
                          ${openAccordion === faq.id ? 'rounded-t-0 ps-5 px-10 bg-white grid-rows-[1fr] opacity-100 rounded-b-xl pb-5 border-s-8 border-t-0 border-b-0 border-e-0' : 'grid-rows-[0fr] opacity-0'}`}
                  >
